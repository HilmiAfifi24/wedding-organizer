...
const buildBookingListQuery = (params: {
  ...
  if (params.filters.user?.trim()) {
    query.set("user", params.filters.user.trim());
  }
...
export const bookingsApi = {
  list: (params: { page: number; pageSize: number; filters: BookingListFilters }) =>
    request<PaginatedResult<AdminBookingListItemDTO>>(
      `/api/admin/bookings?${buildBookingListQuery(params)}`
    ),

  detail: (bookingId: string) => request<AdminBookingDetailDTO>(`/api/admin/bookings/${bookingId}`),

  history: (bookingId: string) =>
    request<BookingStatusHistoryDTO[]>(`/api/admin/bookings/${bookingId}/history`),

  updateStatus: (bookingId: string, input: UpdateBookingStatusInput) =>
    request<AdminBookingDetailDTO>(`/api/admin/bookings/${bookingId}/status`, {
      method: "PATCH",
