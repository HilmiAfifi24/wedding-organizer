...
export async function GET(request: Request) {
  try {
    ...
    const { getDashboardReviewsOverviewUseCase } = createDashboardOverviewUseCases();
    const data = await getDashboardReviewsOverviewUseCase.execute(actorId, parsed.data.timeRange);
