...
export async function GET(request: Request) {
  try {
    ...
    const { getDashboardPaymentsOverviewUseCase } = createDashboardOverviewUseCases();
    const data = await getDashboardPaymentsOverviewUseCase.execute(actorId, parsed.data.timeRange);
