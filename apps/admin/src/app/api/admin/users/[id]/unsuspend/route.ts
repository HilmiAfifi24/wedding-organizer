...
import { unsuspendUserBodySchema, userIdParamSchema } from "@/modules/users/schemas/user-management";
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const parsedBody = unsuspendUserBodySchema.safeParse(body);
    if (!parsedBody.success) {
      return errorResponse(400, "Invalid request body", parsedBody.error.flatten());
    }

    const { unsuspendAdminUserUseCase } = createUserManagementUseCases();
    const data = await unsuspendAdminUserUseCase.execute(actorId, parsedParams.data.id);
