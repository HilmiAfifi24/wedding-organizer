...
export async function GET(
  ...
  try {
    ...
    const { getAdminPaymentProofDetailUseCase } = createPaymentMonitoringUseCases();
    const data = await getAdminPaymentProofDetailUseCase.execute(actorId, parsedParams.data.id);
