...
export async function POST(request: Request) {
  try {
    ...
    const { updateVendorCoverUseCase } = createVendorProfileUseCases();
    const data = await updateVendorCoverUseCase.execute(actorId, parsed.data.file);

    return successResponse(data, 200, "Cover updated");
  } catch (error) {
