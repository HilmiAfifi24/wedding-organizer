...
export async function PATCH(
  ...
  try {
    ...
    const parsedBody = verifyPaymentProofBodySchema.safeParse(rawBody);

    if (!parsedBody.success) {
      return errorResponse(400, "Invalid request body", parsedBody.error.flatten());
    }

    const { verifyVendorPaymentProofUseCase } = createVendorPaymentManagementUseCases();
    const data = await verifyVendorPaymentProofUseCase.execute(
      {
        actorId,
        vendorId,
